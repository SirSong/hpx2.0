...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'salesOrderModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().salesOrderModule.list;
    ...
    try{
      const resp = await getReq(
        '/proxyBiMai'+'/huaqian/api/v1/salesOrder/search',
        queryParams
      )
...
export  const fetchDetail =  (id) => {
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await getReq(
        '/proxyBiMai'+'/huaqian/api/v1/salesOrder/get/'+id,
        {
        }
      )
      const result = await resp.json();
      ...
      dispatch({
        ...
        payload: {
          ...
          supplier: {
            value: data.supplier || null
          },
          totalMoney: {
            value: data.totalMoney || null
          },
          deposit: {
            value: data.deposit || null
          },
          demanderConState: {
            value: data.demanderConState || null
          },
          supplierConState: {
            value: data.supplierConState || null
          },
          orderStatus: {
            value: data.orderStatus || null
          },
          approvalStatus: {
            value: data.approvalStatus || null
          },
          paymentStatus: {
            value: data.paymentStatus || null
          },
          genContract: {
            value: data.genContract || null
          },
        }
      })
    }catch(e) {
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await deleteReq(
        '/proxyBiMai'+'/huaqian/api/v1/salesOrder/delete/'+id,
        {
        }
      )
      dispatch({
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        id 
        ? await patchReq(
            '/proxyBiMai'+'/huaqian/api/v1/salesOrder/update/'+id,
            {
                ...values,
            }
          )
        : await putReq(
            '/proxyBiMai'+'/huaqian/api/v1/salesOrder/add',
            {
                ...values,
                id: id || '',
            }
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().salesOrderModule.fields,
        ...fields
      }
    })
  }
