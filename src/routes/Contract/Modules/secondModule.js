...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'contractSecondModule::';
...
export const fetchList =  (params) => {
  return async (dispatch, getState) => {
    ...
    let list = getState().contractSecondModule.list;
    ...
    try{
      const resp = await getReq(
        '/proxyBiMai'+'/huaqian/api/v1/contract/getDetail',
        queryParams
      )
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await deleteReq(
        '/proxyBiMai'+'/huaqian/api/v1/contract/deleteDetail/'+id,
        {
        }
      )
      dispatch({
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        id 
        ? await patchReq(
            '/proxyBiMai'+'/huaqian/api/v1/contract/updateDetail/'+id,
            {
                ...values,
            }
          )
        : await putReq(
            '/proxyBiMai'+'/huaqian/api/v1/contract/addDetail',
            {
                ...values,
                id: id || '',
            }
