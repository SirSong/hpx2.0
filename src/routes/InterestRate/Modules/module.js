...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'interestRateModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().interestRateModule.list;
    ...
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/enterprises/interestRate/search',
        queryParams
      )
...
export  const fetchDetail =  (id,eid,pid) => {
  return async (dispatch, getState) => {
    if(!id){//没有id，就是新增
      dispatch({
        type: UPDATE_FIELDS,
    ...
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/interestRate/enterprises/'+eid+'/projectId/'+pid,
        {
        }
      )
      const result = await resp.json();
      ...
      dispatch({
        ...
        payload: {
          ...
          projectId: {
            value: data.projectId || null
          },
          annual: {
            value: data.annual || null
          },
          defaultInterest: {
            value: data.defaultInterest || null
          },
          moratorium: {
            value: data.moratorium || null
          },
          lineOfCredit: {
            value: data.lineOfCredit || null
          },
          creditPeriodBegin: {
            value: data.creditPeriodBegin || null
          },
          creditPeriodOver: {
            value: data.creditPeriodOver || null
          },
          marginRatio: {
            value: data.marginRatio || null
          },
          stopLossLine: {
            value: data.stopLossLine || null
          },
          marginCalls: {
            value: data.marginCalls || null
          },
          deliveryDeadline: {
            value: data.deliveryDeadline || null
          },
          remainingCredit: {
            value: data.remainingCredit || null
          },
        }
      })
    }catch(e) {
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        id 
        ? await patchReq(
            '/proxyHq'+'/huaqian/api/v1/interestRate/'+id,
            {
                ...values,
            }
          )
        : await postReq(
            '/proxyHq'+'/huaqian/api/v1/interestRate',
            {
                ...values,
                id: id || '',
            }
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().interestRateModule.fields,
        ...fields
      }
    })
  }
