...
class Compo extends React.Component {
  ...
  getUrlQueryParams(v) {
    ...
    return '/PurchaseContract/' + encodeURI(JSON.stringify(v || {}));
  }
  render() {
    ...
    let view = <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <Link 
          to={`${this.getUrlQueryParams()}/receiving/`+this.state.selectedRowKeys[0]} 
          style={{ marginRight: '16px' }}
        >
          <Button type="primary" disabled={this.state.selectedRowKeys.length !== 1}>确认收货</Button>
         </Link>
      </div>
      <Table
        rowKey="id"
