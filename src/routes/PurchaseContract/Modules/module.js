...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'purchaseContractModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().purchaseContractModule.list;
    ...
    let firstParty = queryParams.firstParty ? queryParams.firstParty.value : null;
    if(firstParty) queryParams.firstParty = queryParams.firstParty.value;
    else delete queryParams.firstParty;
    ...
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/contract/listC',
        queryParams
      )
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().purchaseContractModule.fields,
        ...fields
      }
    })
  }
