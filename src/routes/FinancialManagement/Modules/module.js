...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'financialManagementModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().financialManagementModule.list;
    let queryParams = Object.assign({},list.queryParams);
    
    let tranDate = queryParams.tranDate ? queryParams.tranDate.value : null;
    delete queryParams.tranDate;
    if(tranDate){
      if( tranDate[0] ) queryParams.startKeyName = tranDate[0];
      if( tranDate[1] ) queryParams.endKeyName = tranDate[1];
    }
      
    let tranType = queryParams.tranType ? queryParams.tranType.value : null;
    if(tranType) queryParams.tranType = queryParams.tranType.value;
    else delete queryParams.tranType;
    ...
    try{
      const resp = await getReq(
        '/core'+'/core/api/v1/financialManager/1/accountStatement',
        queryParams
      )
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().financialManagementModule.fields,
        ...fields
      }
    })
  }
