    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        //在提交前格式化部分数据
          ...
          values.applicationDate = values.applicationDate ? values.applicationDate.format('YYYY/MM/DD') : '';
    ...
    return (
      <Form onSubmit={this.handleSubmit}>
          ...
          <FormItem
            ...
            hasFeedback
          >
            {getFieldDecorator('receiptsType',{
              rules: [
                { required: true, message: '请选择!'},
              ],
            })(
              <Select placeholder="请选择" >
                {[{"value":"退款单","name":"T"},{"value":"补发单","name":"F"},{"value":"补购单","name":"G"}].map(
          ...
          >
            {getFieldDecorator('approvalStatus',{
              ...
              <Select placeholder="请选择" >
                {[{"value":"待审批","name":"0"},{"value":"已通过","name":"1"},{"value":"已拒绝","name":"2"}].map(
                  (v) => <Option key={v.value} value={v.value}>{v.name}</Option>
                )}
              </Select>
            )}
          ...
          >
            {getFieldDecorator('paymentStatus',{
              ...
              <Select placeholder="请选择" >
                {[{"value":"待支付","name":"0"},{"value":"已支付","name":"1"}].map(
                  (v) => <Option key={v.value} value={v.value}>{v.name}</Option>
                )}
              </Select>
            )}
          ...
          <FormItem
            ...
            hasFeedback
          >
            {getFieldDecorator('shipmentsStatus',{
              rules: [
                { required: true, message: '请选择!'},
              ],
            })(
              <Select placeholder="请选择" >
                {[{"value":"待收货","name":"0"},{"value":"已收货","name":"1"}].map(
                  (v) => <Option key={v.value} value={v.value}>{v.name}</Option>
                )}
              </Select>
            )}
  ...
  mapPropsToFields(props) {
    var arr = [{
          ...
          value: {
            ...
            value: props.applicationDate && props.applicationDate.value ? moment(props.applicationDate.value) : null,
          }
        },{
          key: 'paymentStatus',
          value: {
