    ...
    this.columns = [{
        ...
        */{
        ...
        },
        {
          ...
          render: (text, record) => 
            <span>{
              [{"value":"0","name":"正常"},{"value":"1","name":"逾期"},{"value":"2","name":"待处置"},{"value":"3","name":"已处置"}]
                .filter((v)=> v.value == text).map((v)=>v.name) || ''
            }</span>
        },
        {
    ...
    let view = <div>
      <div style={{ marginBottom: 16 }}>
       ...
       <Link style={{ marginRight: '30px'}}>
          <input type="text" value="查询合同编号"></input>
        </Link>
        <Link style={{ marginRight: '20px' }}>
            <select name="selectAge" id="selectAge" value="选择供应商">   
              <option value="上海邻客网络科技有限公司">上海邻客网络科技有限公司</option>      
            </select>   
        </Link>
        <br></br>
        <br></br>
        ...
        <Link 
          to={`${this.getUrlQueryParams()}/financing/`+this.state.selectedRowKeys[0]} 
          style={{ marginRight: '16px' }}
        >
          <Button type="primary" disabled={this.state.selectedRowKeys.length !== 1}>查看融资详情</Button>
         </Link>
         <Link 
          to={`${this.getUrlQueryParams()}/detail/0`} 
          style={{ marginRight: '16px' }}
        >
          <Button type="primary">已处置</Button>
         </Link>
