...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'salesContractModule::';
...
export const FETCH_FINANCING_NOT = ACTION_PREFIX + 'FETCH_FINANCING_NOT'
export const FETCH_FINANCING_REQUEST = ACTION_PREFIX + 'FETCH_FINANCING_REQUEST'
export const FETCH_FINANCING_FAILURE = ACTION_PREFIX + 'FETCH_FINANCING_FAILURE'
export const FETCH_FINANCING_SUCCESS = ACTION_PREFIX + 'FETCH_FINANCING_SUCCESS'
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().salesContractModule.list;
    ...
    let secondParty = queryParams.secondParty ? queryParams.secondParty.value : null;
    if(secondParty) queryParams.secondParty = queryParams.secondParty.value;
    else delete queryParams.secondParty;
    ...
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/contract/listS',
        queryParams
      )
...
export const fetchFinancing =  (id) => {
  return async (dispatch, getState) => {
    dispatch({
      type    : FETCH_FINANCING_REQUEST,
    })
    let list = getState().salesContractModule.list;
    
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/contract/queryFinancingSituation/'+id,
      )
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().salesContractModule.fields,
        ...fields
      }
    })
  }
...
const ACTION_HANDLERS = {
  ...
  [FETCH_FINANCING_REQUEST] : (state, action) => 
    ({ 
      ...state, 
      listStatus : { 
        type : FETCH_FINANCING_REQUEST,
  ...
  [FETCH_FINANCING_SUCCESS] : (state, action) => 
    ({ 
      ...state, 
      listStatus : { 
        type : FETCH_FINANCING_SUCCESS,
  ...
  [FETCH_FINANCING_FAILURE] : (state, action) => 
    ({ 
      ...state, 
      err: action.payload,
      listStatus : { 
