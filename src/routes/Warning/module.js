...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'userManagerModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().userManagerModule.list;
    ...
    try{
      const resp = await getReq(
        '/proxyCore'+'/core/api/v1/enterprise/1/users',
        queryParams
      )
...
export  const fetchDetail =  (id) => {
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await getReq(
        '/proxyCore'+'/core/api/v1/enterprise/1/users/'+id,
        {
        }
      )
      const result = await resp.json();
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().userManagerModule.fields,
        ...fields
      }
    })
  }
