...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'myOrderModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().myOrderModule.list;
    ...
    let supplier = queryParams.supplier ? queryParams.supplier.value : null;
    if(supplier) queryParams.supplier = queryParams.supplier.value;
    else delete queryParams.supplier;
    ...
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/salesOrder/search',
        queryParams
      )
...
export  const fetchDetail =  (id) => {
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/salesOrder/get/'+id,
        {
        }
      )
      const result = await resp.json();
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await deleteReq(
        '/proxyHq'+'/huaqian/api/v1/salesOrder/delete/'+id,
        {
        }
      )
      dispatch({
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        id 
        ? await patchReq(
            '/proxyHq'+'/huaqian/api/v1/salesOrder/update/'+id,
            {
                ...values,
            }
          )
        : await putReq(
            '/proxyHq'+'/huaqian/api/v1/salesOrder/add',
            {
                ...values,
                id: id || '',
            }
