...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'myOrderSecondModule::';
...
export const fetchList =  (params) => {
  return async (dispatch, getState) => {
    ...
    let list = getState().myOrderSecondModule.list;
    ...
    // console.log(params.tSalesId)
    try{
      const resp = await getReq(
        '/proxyHq'+'/huaqian/api/v1/salesOrder/getDetail/'+params.tSalesId,
        // queryParams
        {}
      )
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        id 
        ? await patchReq(
            '/proxyHq'+'/huaqian/api/v1/salesOrder/updateDetail/'+id,
            {
                ...values,
            }
          )
        : await putReq(
            '/proxyHq'+'/huaqian/api/v1/salesOrder/addDetail',
            {
                ...values,
                id: id || '',
            }
